 ...
 * 
 *         This class is able to execute algorithms from external classes using
 *         Java's reflection capabilities.
 * 
 *         The goal is to measure the execution time of those algorithms
 *         executing them using increasing amounts of workload (n).
 * 
 *         The resuts are stored in CSV files able to be later processed by
...
public class TestBench {
	...
	/**
	 * Executes a method in an external class a number of times calculating the
	 * average execution time for all the runs, which is stored in a CSV file (one
	 * entry per workload).
	 * 
	 * The workload for each execution set can be defined including the initial and
	 * final workload.
	 * 
	 * 
	...
	/**
	 * Executes a method in an external class making use of the Java's reflection
	 * capabilities.
	 * 
	 * @param className  Name of the class
	 * @param methodName Name of the method
	 * @param n          Workload for the method (called methods MUST accept a
	 *                   single int value as a parameter
	 */
	 ...
	 * @param methodName Name of the method
	 * @param n          Workload for the method (called methods MUST accept a
	 *                   single long value as a parameter
	 */
	 ...
	 * 
	 * @param i Prints out the iteration number in the console (information to the
	 *          user)
	 * @return
	 */

	/*
	 * public static void doNothing(long i) { System.out.println
	 * ("Doing nothing at step... ("+i+")");
	 * 
	 * long endTime = System.currentTimeMillis() + SLEEP_TIME; while
	 * (System.currentTimeMillis() < endTime) { // do nothing } }
	 */
...
//			System.out.println();			
			...
			// to store mediations, sample, initial value of n, last value of n,
			// package.Class, method to test
			// O(n)- n=50, O(n^2) lower, O(log(n)) higher
			// time in ms
			TestBench.test("pow2LogarithmicAndExponential.txt", 6, 0, 50, "uo.ed.p1.recursion.Algorithms",
					"pow2LogarithmicAndExponential");
			System.out.println();
